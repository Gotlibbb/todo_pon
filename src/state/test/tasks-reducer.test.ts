...
test('new array should be added when new todolist is added', () => {
    const action = addTodolistAC({title: "new todolist", order: 0, addedDate: "", id: "123", });
...
test('set todo task', () => {
    const action = setTodolistAC([{id: "tdl", title: "What to pay", addedDate: "", order: 0}]);

    const endState = tasksReducer({}, action)

    const keys = Object.keys(endState);

    expect(endState["tdl"]).toEqual([]);
});
